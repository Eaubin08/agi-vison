...
```mermaid
graph TD
    A[Noyau Cognitif - IA] -->|Proposition| B{Espace Latent}
    B -->|Analyse de Friction| C[Agents Périphériques]
    C -->|Feedback| B
    B -->|Structure Stabilisée| D{Juge Déterministe}
    D -->|Audit X-108| E[Invariants Lean 4]
    E -->|Validation| F[Mémoire Scellée]
    E -->|Veto| G[Purge Ontologique]
    G --> B
```
